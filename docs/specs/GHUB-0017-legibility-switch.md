...
| fold-back | 2026-09-08 | implementation (GHUB-0153, Canasta's layout figures) | — | — | — | — | **The minimum size the 2026-08-19 row records was solved against one shape, and the table lost width at others.** That row says `minimumSizeHint()` returns 900×656 with both terms of `cardWidth()` solving to ~62.7 there, and it was right about that shape. It is not right about every shape the window can take. `tableRect()` insets by 2.2% of the SHORTER side, so once a window is taller than it is wide the shorter side is the WIDTH and the inset stops shrinking with height — the usable width falls as the window grows. Solving the width term against that worst inset (0.072 × 0.956 × width) needs 904, not 893, so the minimum is now **908×656**. Measured before the change: `cardsFitTable()` was false at 900×1000 and 900×1400, and 900×1000 is the shape the README promises fits beside your work. The height term has no such coupling and is unchanged. **§4.4's obligation is not breached** — raising the minimum alone makes the floor more redundant, which is the direction that section wants; the floor and `kMeldScale` are untouched and Canasta's smallest meld card is still 46.4 px, because the height term is what binds at the minimum. **INV-3's assertion was the thing at fault, not the arithmetic**: it asked `cardsFitTable()` at the minimum alone, where the shorter side is the height, and was blind to every taller shape. It now asks across a spread of them, and was seen red at 900×1000 against the unraised constant. |
